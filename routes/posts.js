...
module.exports = {  //Exports the CRUD requests 
    get(req, res) { // Shows to client all the existing blog posts
        res.status(200).send(req.store.posts);  // "OK" code is emmitted and send to client all the blog posts stored
    }, 
    add(req, res) { // Adds a new blog post
        ...
        let id = req.store.posts.length;  // Saves the amount of posts as the postId - Post 1 has postId=0 and so on
        let newPost = {                   // Only takes the information that is necessary - Input cleaning
          name: req.body.name,
          url: req.body.url,
          text: req.body.text
        };
        req.store.posts.push(newPost);      // Saves the post
        res.status(201).send({postId: id}); // "Created" code is emmitted and sends to client the postId number
    },
    update(req, res) { // Updates an existing blog post
        ...
        req.store.posts[req.params.postId] = newPost;             // Updates the selected post
        res.status(200).send(req.store.posts[req.params.postId]); // Emmits "Ok" code and sends to client the updated post
    },
    remove(req, res) { // Deletes a post
        if (!req.store.posts[req.params.postId]) {                        // Checks if the post exists
          ...
          req.store.posts.splice(req.params.postId, 1);           // Deletes the selected post
